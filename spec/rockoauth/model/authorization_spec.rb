...
describe RockOAuth::Model::Authorization do
  ...
  describe "when there are existing authorizations" do
    ...
    describe ".create_code" do
      before { RockOAuth.stub(:random_string).and_return('existing_code', 'new_code') }

      it "returns the first code the client has not used" do
        RockOAuth::Model::Authorization.create_code(client).should == 'new_code'
      end

      it "returns the first code another client has not used" do
        RockOAuth::Model::Authorization.create_code(impostor).should == 'existing_code'
      end
    end

    describe ".create_access_token" do
      before { RockOAuth.stub(:random_string).and_return('existing_access_token', 'new_access_token') }

      it "returns the first unused token it can find" do
        RockOAuth::Model::Authorization.create_access_token.should == 'new_access_token'
      end
    end

    describe ".create_refresh_token" do
      before { RockOAuth.stub(:random_string).and_return('existing_refresh_token', 'new_refresh_token') }

      it "returns the first refresh_token the client has not used" do
        RockOAuth::Model::Authorization.create_refresh_token(client).should == 'new_refresh_token'
      end

      it "returns the first refresh_token another client has not used" do
        RockOAuth::Model::Authorization.create_refresh_token(impostor).should == 'existing_refresh_token'
      end
    end
  ...
  describe "#exchange!" do
    ...
    it "uses its helpers to find unique tokens" do
      RockOAuth::Model::Authorization.should_receive(:create_access_token).and_return('access_token')
      authorization.exchange!
